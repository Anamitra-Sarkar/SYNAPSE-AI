...
The supplied Firebase project credentials are configured as environment values and the Admin SDK credential has been verified with a read-only Firebase Auth call. Before end users can sign in and create projects, open the Firebase console for `synapse-ai-98e8a` and complete the following production activation steps.
...
| Service account | Keep the supplied service-account credential only in Vercel’s server-side environment configuration. Rotate it immediately if it has ever been committed, shared publicly, or exposed outside the Firebase console and secure project configuration. |

The React client uses only Firebase’s public web configuration. Morrow project documents are written to the authenticated user’s `projects` collection, while the Firestore rules restrict access to the document owner. Groq remains a server-side concern and must continue to use a Vercel server-only secret.
