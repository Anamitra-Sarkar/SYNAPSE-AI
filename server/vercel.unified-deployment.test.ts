...
describe("unified Vercel deployment", () => {
  it("keeps the Vite frontend and the catch-all API handler in one Vercel project", async () => {
    const [vercelConfig, apiHandler, trpcHandler, apiApp] = await Promise.all([
      readFile(resolve(process.cwd(), "vercel.json"), "utf8"),
      readFile(resolve(process.cwd(), "api/[...path].js"), "utf8"),
      readFile(resolve(process.cwd(), "api/trpc/[...procedure].js"), "utf8"),
    ...
    expect(vercelConfig).toContain('"src": "/api/trpc/(.*)", "dest": "/api/trpc/[...procedure].js"');
    expect(vercelConfig).toContain('"src": "/api/(.*)", "dest": "/api/[...path].js"');
