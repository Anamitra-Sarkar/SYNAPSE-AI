...
export async function saveBlueprintArtifacts(projectId: string, ownerId: string, immutableOutput: unknown, userEdits: unknown, blueprintId: string, selectedConceptId: string) { await setDoc(doc(db(), "projects", projectId, "blueprints", "latest"), { ownerId, immutableOutput, userEdits, blueprintId, selectedConceptId, updatedAt: serverTimestamp() }); }
